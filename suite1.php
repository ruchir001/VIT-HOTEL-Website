<html>
<head>
	<title>suite1</title>
</head>
<body bgcolor="#fff2e5">
	<?php ?>
	<h3 align=center><font color=chocolate>SUITE ROOM</font></h3>
	<center><a href=suite.php><img src="image/suite.jpeg" width=340 height=298></a>
	</center>
	<table border=0>
	<tr>
		<th><font size=4>Location:2<sup>nd</sup>floor</th>
	</tr>
	<tr>
		<th><font size=4>size:23<sup>'</sup>0<sup>"</sup>*13<sup>'</sup>6<sup>"</sup></th>
	</tr>
	</table>
	<br><br>
	<table align=center>
	<tr>
		<td>
		<ul type=square>
			<li><font color="#7c0000" size=4>Room</font>
			<font color="choco">
		<ul type=disc>
			<li>Queen/Twin bed.
			<li>Individually controolled air conditioning.
			<li>direct access internet.
			<li>29<sup>th</sup>flat interactive television.
			<li>Electronic door locks.
			<li>Satelight entertainment & new channels.
			<li>Fully stocked minibar.
			<li>Direct IDD telephone with voicemail.
			<li>Laptop compatiable digital safe.
			<li>Marble flooring.
			<li>Data port on telephone.
			<li>Power socket on besides and desk.
			<li>Separate luggage area with luggage rack.
			<li>Daily complimentary newspaper
			<li>Room locar facalities.
		</ul>
		<font color="#7c0000" size=4><li>Bathroom</font>
		<ul type=disc>
			<li>ensuite bathroom
			<li>Bath tubs
			<li>Cosmetic mirror
			<li>Hair dryer
			<li>Weighing scale
			<li>Power socket
			<li>Running hot & cold water in showers
		</ul>
		</ul>
		</font></td></tr>
</table>
		<a href=accommodation.php>home</a>
<marquee behavior=alternate bgcolor="#7e0000"><b><i><font color="white">Devloped By :- Ruchir Bhaskar </i></b></font></marquee>

		</body>
</html>
